...
namespace lab02 {
    public struct Vertex : Pars.IParsable {
        ...
        public Vertex Divide( double factor ) {
            Vertex divided;

            divided._x = _x / factor;
            divided._y = _y / factor;
            return divided;
        }
        ...
        public static Vertex operator /( double factor, Vertex point ) {
            return new Vertex( factor / point._x, factor / point._y );
        }

        public static Vertex operator /( Vertex point, double factor ) {
            return point.Divide( factor );
        }

        public static Vertex operator /( Vertex one, Vertex two ) {
            return new Vertex( one._x / two._x, one._y / two._y );
        }
